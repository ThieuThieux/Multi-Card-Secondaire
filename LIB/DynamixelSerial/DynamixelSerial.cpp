...
// ajout chris
int DynamixelClass::setmultiturn(unsigned char ID)
{

	char AX_CCW_AL_LT = 4095; // Changing the CCW Angle Limits for Full Rotation.
	Checksum = (~(ID + 7 + AX_WRITE_DATA + AX_CW_ANGLE_LIMIT_L + 0xFF + 0x0F + 0xFF + 0x0F)) & 0xFF;
...
int DynamixelClass::setmultiturnv(unsigned char ID)
{

	char AX_CW_AL_LT = 4095; // Changing the CCW Angle Limits for Full Rotation.
	Checksum = (~(ID + AX_GOAL_LENGTH + AX_WRITE_DATA + AX_CW_ANGLE_LIMIT_L + 0xFF + 0x0F)) & 0xFF;
...
int DynamixelClass::stopmultiturn(unsigned char ID)
{

	char AX_CW_AL_LT = 0; // Changing the CCW Angle Limits for Full Rotation.
	Checksum = (~(ID + AX_GOAL_LENGTH + AX_WRITE_DATA + AX_CW_ANGLE_LIMIT_L)) & 0xFF;
...
int DynamixelClass::setPid(unsigned char ID, int P, int I, int D)
{
	Checksum = (~(ID + 5 + AX_WRITE_DATA + AX_CW_COMPLIANCE_MARGIN + D + I + P)) & 0xFF;
	...
	sendData(5);
	sendData(AX_WRITE_DATA);
	sendData(26); // D register
	sendData(D);
	sendData(I);
	sendData(P);
	sendData(Checksum);
	sendData(TX_DELAY_TIME);
	switchCom(Direction_Pin, Rx_MODE);
